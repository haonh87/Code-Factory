...
test("new bundle events require a direct canonical transaction identity; lifecycle events omit it", () => {
  for (const action of bundleActions) {
    ...
    assert.equal(event.note, args.note, "human context is independent of the direct identity");
  }
...
test("explicit malformed persisted identity is rejected, never generated by the compatibility reader", () => {
  const base = { timestamp: "2026-09-13T00:00:00Z", action: "approve-closeout-bundle", actor: "human-review-bundle", from_status: "ACTIVE", to_status: "ACTIVE", note: "Human-only" };
  for (const value of [undefined, "", null, "bad-id", " " + transactionId, [transactionId], { toString: () => transactionId }]) {
    assert.throws(() => utils.normalizeProtocolReport({ protocol_events: [{ ...base, transaction_id: value }] }), /transaction_id|transaction.*identity|UUID/i);
  }
  assert.equal(Object.hasOwn(utils.normalizeProtocolReport({ protocol_events: [base] }).protocol_events[0], "transaction_id"), false, "only absent historical identity remains unbound");
  for (const value of [[transactionId], { toString: () => transactionId }]) assert.throws(() => utils.buildProtocolEvent({ action: base.action, transactionId: value }), /transaction_id|transaction.*identity|UUID/i);
});

...
function testSemanticEvidenceGuardExportedRegisteredAndRuns() {
  assert(
    typeof smoke.runCaseSemanticEvidenceGuard === "function",
    "runCaseSemanticEvidenceGuard must be exported from run-workflow-authoring-smoke"
  );
  const source = fs.readFileSync(
    path.join(__dirname, "..", "scripts", "run-workflow-authoring-smoke.js"),
    "utf8"
  ...
  assert(
    /name:\s*"semantic-evidence-guard"/.test(source),
    "authoring smoke case list must register semantic-evidence-guard"
  );
  if (typeof smoke.runCaseSemanticEvidenceGuard === "function") {
    try {
      smoke.runCaseSemanticEvidenceGuard(path.resolve(__dirname, ".."));
    } catch (error) {
      failures += 1;
      console.error(`  FAIL: semantic evidence guard threw: ${error.message}`);
    }
  }
  console.log("  PASS: semantic evidence guard exported, registered, and runnable");
}
...
testSemanticEvidenceGuardExportedRegisteredAndRuns();

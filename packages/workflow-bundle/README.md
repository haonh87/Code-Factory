...
`workflow-bundle` is the CLI package of the `v2.3.1` public release: it installs the workflow bundle for Codex or Claude Code, scaffolds or validates workflows, and supports the `agent proposes, human approves` flow for `work-item` and `change`.

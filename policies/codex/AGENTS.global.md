...
At steps `s01` to `s04`, before acceptance criteria are locked and before any technical approach is chosen, use `sa` and `ta` to turn the request into architecture drivers: `sa` carries the solution and system lens — objectives with measures, business and regulatory constraints, which system should own which capability, where the seams between systems sit — and `ta` carries the technical lens — quality attribute scenarios with numeric thresholds, technical constraints, integration reality and legacy limits. Both run standalone from a raw request; `requirement-analysis` and `product-thinking` output is optional supplementary input, not a precondition. Neither chooses a technical approach, which stays with `system-design` at `s05`, and neither designs the inside of a system, which stays with `domain-architecture`.

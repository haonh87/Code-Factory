...
The entry router's role-applicability result is authoritative for step-skill selection. Use `sa` and `ta` only when the entry router selects them as applicable at `s01` to `s04`, before acceptance criteria are locked and before any technical approach is chosen. If the router omits a role or marks it `not_applicable`, generic skill guidance must not re-add that role. When applicable, `sa` turns the request into solution and system architecture drivers — objectives with measures, business and regulatory constraints, capability ownership, and system seams — while `ta` turns it into technical architecture drivers — quality attribute scenarios with numeric thresholds, technical constraints, integration reality, and legacy limits. Both run standalone from a raw request; `requirement-analysis` and `product-thinking` output is optional supplementary input, not a precondition. Neither chooses a technical approach, which stays with `system-design` at `s05`, and neither designs the inside of a system, which stays with `domain-architecture`.

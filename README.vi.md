...
- `list|status` chỉ đọc và không persist report. `approve` tường minh có thể bootstrap work item chỉ có scaffold và ghi `request_source=legacy-scaffold`, `REPORT_BOOTSTRAPPED` cùng reviewer human; `activate|verify|close` vẫn yêu cầu `.work-item-report.json` đã tồn tại.

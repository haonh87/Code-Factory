...
Tài liệu này ghim bề mặt phát hành công khai dự kiến cho `workflow-bundle v2.6.1`.

`v2.6.1` là một ứng viên phát hành và vẫn chưa được phát hành cho đến khi human Release gate phê duyệt. Khi được phê duyệt, public surface dự kiến sẽ:
...
`v2.0.0` vẫn là first public release. Nếu được phê duyệt, `v2.6.1` tiếp tục trên cùng public surface và không tạo breaking change cho command line:
...
- Tag sau Release approval: `v2.6.1`
- Candidate evidence trước approval: source commit và immutable tarball digest ghi trong CHANGE-006
...
Hai lớp legacy trên hiện vẫn được giữ để migration êm hơn, nhưng không nên coi đó là core public story của `v2.6.1`.
...
- Trước publication, rollback chỉ khôi phục các source surface và candidate surface do CHANGE-006 quản lý về baseline `v2.6.0/42` đã xác minh.
- Sau publication được phê duyệt, dùng artifact v2.6.0 bất biến đã lưu và `wfc install` để hạ cấp có quản lý, xác minh `v2.6.0/42`, đồng thời giữ nguyên file và mode unmanaged.
...
- cho tới khi Release được phê duyệt, gọi `v2.6.1` là ứng viên phát hành thay vì public release hiện tại

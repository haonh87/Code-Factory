...
This document pins the public publish surface for `workflow-bundle v2.3.1`.

`v2.3.1` is the current public release of this workflow:
...
`v2.0.0` remains the first public release. `v2.3.1` is the next release on the same public surface, and it makes no breaking change:
- `v2.2.x` added the harness adapter registry and internal hooks/instincts tooling.
- `v2.3.1` adds an English-first public surface (with Vietnamese `*.vi.md` supplements) and an `sdd_mode=light` profile for lower-ceremony work items.
...
The two legacy layers above are still kept to make migration smoother, but they should not be treated as the core public story of `v2.3.1`.
...
- when talking about the public release, use the language of `v2.3.1`

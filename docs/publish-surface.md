...
This document pins the public publish surface for `workflow-bundle v2.2.1`.

`v2.2.1` is the current public release of this workflow:
...
`v2.0.0` remains the first public release; `v2.2.1` is the next release on the same public surface, with no breaking change (`v2.2.0` added the harness adapter registry — additive; `v2.2.1` only adds internal repo tooling, no change to the public promise below).
...
The two legacy layers above are still kept to make migration smoother, but they should not be treated as the core public story of `v2.2.1`.
...
- when talking about the public release, use the language of `v2.2.1`

...
This repository stores the policy, workflow, skills, and adapters for AI agent tasks. The current public release is `workflow-bundle v2.3.2`: an installable workflow bundle for Codex and Claude Code that lets the agent proactively propose `work-item` and `change`, while the human retains approval authority at the gates before delivery proceeds.

When sharing with new users, use the `v2.3.2` tag or the `release/v2.3.2` branch as the canonical public reference instead of the working tree.
...
If you are approaching the repo for the first time and want to follow the `v2.3.2` public release:
...
The public command surface of `v2.3.2` uses `wfc`.
...
- The approval model of `v2.3.2` is `agent proposes, human approves`; `ACTIVE` opens only when the approval gate, trusted signed receipts, and the required step-gate evidence are present.
...
- Public publish surface for `v2.3.2`: [`docs/publish-surface.md`](docs/publish-surface.md)

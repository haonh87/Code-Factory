...
> [!summary]
> QC opened TS8/s08 for reviewed code `04eed2f8b2098bddf513d0f96fd129e835686dd7`. Local candidate45/artifact4/rollback4 PASS per Node18/22.
> User accepted TS8-M1 metadata-only Node24 card repair; exact pre/post equivalence and all six validators pass.
> Current protocol ACTIVE at s08 for hosted rerun. Failed run34800821538 retained as history; no hosted candidate or terminal approval inferred. Scan PARTIAL; mandatory child/parent exact-candidate verification retained.
...
release_blockers: ["Hosted exact candidate and required Node18/22 matrix job evidence pending","Mandatory AC-RCR-08 parent contribution and independent terminal gates"]
status: "PARTIAL"
...
recommendation: "Execute existing hosted Verify for the metadata-corrected commit; bind actual full candidate SHA-256 before separate QC Technical Verification."
...
checks: ["TS8-M1 Developer/QC accepted exact one-card metadata path","Card six scalars normalized, five origins preserved, all other bytes identical","Five frozen RCR authoring receipts and reviewed source hashes unchanged","All six source/artifact validator wrappers PASS"]
blocking_items: ["Hosted rerun/candidate proof pending"]
owner: "qc"
next_action: "Existing hosted exact-candidate Verify"
```
...
gaps: ["Hosted exact candidate and required Node18/22 matrix job evidence pending","Mandatory AC-RCR-08 parent contribution and independent terminal gates"]
residual_risks: ["Native transient-read refusal and linear history cost are retained advisories"]
follow_up_items: ["Mandatory AC-RCR-08 parent exact-candidate AG-01..13 and terminal decisions"]
next_action: "Execute existing hosted Verify for the metadata-corrected commit; bind actual full candidate SHA-256 before separate QC Technical Verification."
```
...
- Status: ACTIVE at s08; accepted metadata correction completed, hosted rerun pending.
- Next: build once on hosted CI and bind source/run/full checksum; consume the same bytes on Node18/22.
- Parent contribution mandatory; scan PARTIAL and F-AG11-001 OPEN. No terminal gate approved.
...
notes_for_implementation_or_ops: "Accepted TS8-M1 metadata-only card repair passes six validator wrappers; existing CI topology untouched. Hosted rerun will identify actual source/run/artifact, not reuse old failure as proof."
```
